...
<head>
    ...
    $cash_tendered = 0;
    $cashier = '';
    $pt = 'cash'; // Default payment type
    $total_amount = 0;
    $change_amount = 0;

    // Get values from URL parameters (from checkout)
    if (isset($_GET['cash'])) {
        $cash_tendered = floatval($_GET['cash']);
    }
    if (isset($_GET['total'])) {
        $total_amount = floatval($_GET['total']);
    }
    if (isset($_GET['change'])) {
        $change_amount = floatval($_GET['change']);
    } else if ($cash_tendered > 0 && $total_amount > 0) {
    ...
    for ($i = 0; $row = $result->fetch(); $i++) {
        ...
        // If no URL parameters, fall back to database values
        if ($cash_tendered == 0) {
            $cash_tendered = $row['due_date'];
        }
        if ($total_amount == 0) {
            $total_amount = $row['amount'];
        }
        if ($change_amount == 0 && $pt == 'cash') {
            $change_amount = $cash_tendered - $total_amount;
        }
    }

    // If no date found in database, use current date
    if (empty($date)) {
        $date = date('Y-m-d');
    }

    // If no cashier from URL parameter, get from session
    if (empty($cashier) && isset($_SESSION['SESS_MEMBER_ID'])) {
        $cashier = $_SESSION['SESS_MEMBER_ID'];
    }
    // If still no cashier, set default
    if (empty($cashier)) {
        $cashier = 'Cashier';
    }
    ?>
